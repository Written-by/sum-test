...
TEST(Mac, ctorTest){
	...
	Mac mac2{mac1};
	Mac mac3(_temp);
	Mac mac4(std::string("001122-334455"));
	EXPECT_EQ(mac3, mac4);
	Mac mac5("001122-334455");
	EXPECT_EQ(mac3, mac5);
}

TEST(Mac, castingTest){
	Mac mac("001122-334455");
	const uint8_t* uc = (uint8_t*)mac;
	uint8_t temp[Mac::SIZE];
	for (int i=0; i<Mac::SIZE; i++) temp[i]=*uc++;
	EXPECT_TRUE(memcmp(&mac, temp, 6)==0);
	std::string s2=std::string(mac);
